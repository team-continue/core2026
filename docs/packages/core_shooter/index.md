...
!!! note "今回未使用の入力"
    `magazine_manager` はマガジン充填高さセンサ（`{side}/distance`）とディスクホールドの手動オーバーライド（`{side}/disk_hold_state`）を購読できますが、実機でこれらを発行する構成が今回はないため上図には含めていません。残弾管理・ホールド制御は発射カウントのみで自律的に行われます。詳細は[magazine_managerのノードページ](magazine_manager.md#距離センサによる高さ推定について)を参照してください。
