...
構成は昨年度と同様に、板金、3Dプリンター製ローラー、モータで構成されています。<br>
...
しかし、現状の設計だとまだジャムる場合があるため設計変更が必要です。<br>
...
`core_shooter/config/shooter.params.yaml` の `/**/<side>/aim_bot` で左右それぞれに設定される値です。

| 項目 | 左砲塔 | 右砲塔 |
|------|-------|-------|
| Pitch モータID | 7（Feetech） | 11（Feetech） |
| Yaw モータID | 5（RoboStride 05） | 6（RoboStride 05） |
| Pitch 可動範囲 | `-2.0` ～ `3.14` rad | `-1.8` ～ `3.14` rad |
...
| 項目 | 値 | 説明 |
|------|-----|------|
| `image_width` / `image_height` | 320.0 / 320.0 | 追尾計算で前提とする画像サイズ |
| `image_tolerance_x` / `_y` | 5.0 / 5.0 | 目標到達とみなすピクセル許容誤差 |
| `image_shoot_tolerance_x_px` / `_y_px` | 50.0 / 50.0 | 発射可と判定するピクセル許容誤差 |
...
| 項目 | 値 | 定義元 |
|------|-----|-------|
| 発射モータ（左 / 右） | ESC ID 15 / 16 | `shooter.params.yaml` |
