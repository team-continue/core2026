...
namespace core_costmap_builder
{
  ...
  private:
    ...
    // ====== TF Utils ======
    bool get2DTranslation(const std::string &target, const std::string &source, double &x, double &y);
    bool transformPointCloudToFrame(const sensor_msgs::msg::PointCloud2 &in,
    ...
    // ====== Publish ======
    void publishLocal();
    void publishFilteredPoints(const std::vector<float> &xs,
