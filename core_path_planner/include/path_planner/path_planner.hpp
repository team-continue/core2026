...
class PathPlanner {
 public:
  ...
  PlanResult plan(const nav_msgs::msg::OccupancyGrid& map,
 ...
 private:
  ...
  bool computePath(const nav_msgs::msg::OccupancyGrid& map,
                   const GridIndex& start, const GridIndex& goal,
                   std::vector<GridIndex>& out_path) const;
  void reconstructPath(const nav_msgs::msg::OccupancyGrid& map,
                       const std::vector<int>& came_from, int goal_index,
                       std::vector<GridIndex>& out_path) const;
  double heuristic(const GridIndex& a, const GridIndex& b) const;
  bool worldToGrid(const nav_msgs::msg::OccupancyGrid& map, double wx,
                   double wy, GridIndex& cell) const;
  bool isOccupied(const nav_msgs::msg::OccupancyGrid& map,
