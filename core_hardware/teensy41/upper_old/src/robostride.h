...
struct RoboStrideRef {
    ...
    float vel_kp = 0.f;        // motion kp
    float vel_ki = 0.f;        // motion ki
    float pos_kp = 0.f;        // motion kp
    ...
    bool receive_status_frame(){
        ...
        while ((millis() - start_time) < timeout_ms) {
            ...
            if (result.communication_type == Communication_Type_MotorRequest) {
                ...
                feedback.position_rad   = uint_to_float(pos_u16, -(float)op.position, (float)op.position, 16);
