...
HUDQuadrantElevation::HUDQuadrantElevation(QWidget *parent)
:   QWidget(parent),
    ...
    normalized_angle(0)
{
    ...
    setFixedSize(80,300);
    move(this->parentWidget()->width() - size().width() - 0, this->parentWidget()->height() / 2 - size().height()/2 + 80);
    setStyleSheet(
        "\
        background-color: rgba(0,0,0,0);\
        "
...
void HUDQuadrantElevation::paintEvent(QPaintEvent *event) {
    Q_UNUSED(event);
    
    int view_offset_x = this->width() / 2 - 30;
    int view_offset_y = this->height() / 2;
...
void HUDQuadrantElevation::setValue(float v) {
    std::stringstream ss;
    int n = v / 360;
    float angle = v - n * 360;
