...
void MppiController::setParams(const MppiParams & params)
{
  params_ = params;
}
...
double MppiController::sqr(double x)
{
  return x * x;
}
...
MppiController::State MppiController::rolloutStep(
  const State & s,
  const geometry_msgs::msg::Twist & u) const
{
...
double MppiController::occupancyCost(
  const nav_msgs::msg::OccupancyGrid * map, double x,
  double y) const
{
  ...
  geometry_msgs::msg::Twist & out_cmd)
{
  ...
  for (int k = 0; k < samples; ++k) {
    ...
    for (int t = 0; t < horizon; ++t) {
      ...
      cost += params_.w_smooth *
        (sqr(u.linear.x - u_prev.linear.x) + sqr(u.linear.y - u_prev.linear.y) +
